...
Stateless Ollama REST API service running Phi-3 Mini (3.8B parameters, Q4_0 quantized) for the ai-ml namespace.
...
| Property | Value |
|----------|-------|
| Internal Service | `ollama.ai-ml.svc.cluster.local:11434` |
...
### Chat Completion
```bash
curl -X POST http://ollama.ai-ml:11434/api/chat \
...
### Generate (Simple)
```bash
curl -X POST http://ollama.ai-ml:11434/api/generate \
  -H "Content-Type: application/json" \
  -d '{
    "model": "phi3",
...
```bash
curl http://ollama.ai-ml:11434/api/tags
```
...
```bash
kubectl run curl-test --image=curlimages/curl -n ai-ml --rm -it --restart=Never -- \
